...
class Album(QObject):
    ...
    def _set_info(self, rawAlbumDetails: dict) -> None:
        ...
        assert (
            ...
            or rawAlbumDetails["type"].lower() == "single"
            or rawAlbumDetails["type"].lower() == "ep"
        ), "The provided ID does not correspond to an album or single or EP."
        ...
        self.albumArtists: list[dict[Literal["name", "id"], str]] = rawAlbumDetails[
            "artists"
        ]
        ...
        self.thumbnails: list[
            dict[Literal["url", "width", "height"], Union[str, int]]
        ] = rawAlbumDetails["thumbnails"]

        self.largestThumbnail: dict[
            Literal["url", "width", "height"], Union[str, int]
        ] = self.thumbnails[-1]
        self.largestThumbnailUrl: str = self.largestThumbnail["url"]

        self.smallestThumbnail: dict[
            Literal["url", "width", "height"], Union[str, int]
        ] = self.thumbnails[0]
    ...
    def downloadStatus(self, value: Union[int, DownloadStatus]) -> None:
        self._downloadStatus = (
            DownloadStatus(value) if isinstance(value, int) else value
        )
    ...
    def _set_songs(self) -> Union[list[song.Song], None]:
        if DataStatus(self.dataStatus) is not DataStatus.LOADED:
            self.logger.error(
                "Tried to fetch album's songs, but there is no data available"
            )
            return None
    ...
    def download(self) -> None:
        if len(self.songs) == 0 or DataStatus(self.dataStatus) is not DataStatus.LOADED:
            self.logger.error(
                "Tried to download album's songs, but there is no data available"
            )
            return
...
def albumFromSong(song: song.Song) -> Union[Album, None]:
    ...
    albumInfo = asyncio.run_coroutine_threadsafe(
        afsCoro, universal.asyncBgworker.event_loop
    ).result()
...
def albumFromSongID(songID: str) -> Union[Album, None]:
    albumID = asyncio.run_coroutine_threadsafe(
        _afs(songID), universal.asyncBgworker.event_loop
    ).result()
