...
class DataStoreNotFoundException(Exception):
    ...
    def __init__(
        self,
        message: str = "Data store not found. Please ensure the data store is initialized.",
    ):
...
class CacheNotFoundException(Exception):
    ...
    def __init__(
        self, message: str = "Cache not found. Please ensure the cache is initialized."
    ):
        super().__init__(message)
...
class Song(QObject):
    ...
    def __new__(
        cls, id: str = "", givenInfo: dict = {"None": None}, fs: bool = False
    ) -> "Song":
    ...
    def __init__(
        self, id: str = "", givenInfo: dict = {"None": None}, fs: bool = False
    ):
        """
        ...
        self.playbackReadyChanged.connect(
            lambda: self.logger.debug(
                f"Playback ready changed for song {self.id} ({self.title}): {self.playbackReady}"
            )
        )
    ...
    def _set_info(self, rawVideoDetails: dict) -> None:
        ...
        self.rectangleThumbnail: dict = self.rawMicroformatData[
            "microformatDataRenderer"
        ]["thumbnail"]["thumbnails"][-1]
        self.rectangleThumbnailUrl: str = self.rectangleThumbnail["url"]

        self.fullUrl: str = self.rawMicroformatData["microformatDataRenderer"][
            "urlCanonical"
        ]
        self.description: str = self.rawMicroformatData["microformatDataRenderer"][
            "description"
        ]

        self.tags: Union[list[str], None] = self.rawMicroformatData[
            "microformatDataRenderer"
        ].get("tags", None)

        self.pageOwnerDetails: dict = self.rawMicroformatData[
            "microformatDataRenderer"
        ]["pageOwnerDetails"]
        self.pageOwnerName: str = self.pageOwnerDetails["name"]
        self.pageOwnerChannelId: str = self.pageOwnerDetails["externalChannelId"]

        self.uploadDate: str = self.rawMicroformatData["microformatDataRenderer"][
            "uploadDate"
        ]
        self.publishDate: str = self.rawMicroformatData["microformatDataRenderer"][
            "publishDate"
        ]
        ...
        self.category: str = self.rawMicroformatData["microformatDataRenderer"][
            "category"
        ]

        self.isFamilySafe: bool = self.rawMicroformatData["microformatDataRenderer"][
            "familySafe"
        ]
        ...
        if cachedData := c.get(identifier):  # type: ignore[assignment]
            ...
            if self.rawData.get("playabilityStatus", {}).get("status") == "ERROR":
                raise Exception(
                    ...
                    + self.rawData.get("playabilityStatus", {}).get("reason")
                )
        else:
            return
    ...
    async def get_info(
        self, api: Union[ytm.YTMusic, None] = None, cache_only: bool = False
    ) -> None:
        """
        ...
        if not (cachedData := c.get(identifier)):  # type: ignore[assignment]
            ...
            if self.rawData.get("playabilityStatus", {}).get("status") == "ERROR":
                ...
                return
            if (
                self.rawData.get("playabilityStatus", {}).get("status")
                == "LOGIN_REQUIRED"
            ):
                self.logger.warning(
    ...
    def download_playbackInfo(self) -> None:
        ...
        if not (cachedData := c.get(identifier)):  # type: ignore[assignment]
            ...
            c.put(
                identifier,
                json.dumps(self.rawPlaybackInfo),
                byte=False,
    ...
    def get_playback(self, skip_download: bool = False) -> None:
        ...
        if not datastore:
            self.logger.critical(
                "No data store found for songs, cannot get download info."
            )
            return
        if self.downloadState == DownloadState.DOWNLOADED and not skip_download:
            try:
                ...
                meta: dict = json.loads(
                    cacheManager.getdataStore("song_datastore").get_file(
                        self.id + "_downloadMeta"
                    )
                )
                meta["url"] = fp
        ...
        for format in playbackinfo["formats"]:
            ...
            item["qualityName"] = FMT_DATA_HUMAN.get(
                format["format_id"], item["format_note"]
            )
    ...
    def purge_playback(self):
        ...
        if self.downloadState == DownloadState.DOWNLOADED:
            ...
            cacheManager.getdataStore("song_datastore").delete(
                self.id + "_downloadMeta"
            )
    ...
    async def download_with_progress(
        self, url: str, datastore: cacheManager.dataStore.DataStore, ext: str, id: str
    ) -> None:
        ...
        self.logger.info(
            ...
            {"notifying": True, "customMessage": f"Downloading {self.title}"},
        )
        try:
            ...
            if not success:
                self.logger.warning(
                    f"Download failed for {self.title}, retrying with single-threaded download."
                )
            ...
            if success:
                self.logger.info(
                    f"Download complete for {self.title}",
                    {
                        "notifying": True,
                        "customMessage": f"Download complete for {self.title}",
                    },
                )
    ...
    async def download(self, audio=True) -> None:
        ...
        if audio:
            ...
            self.logger.error(
                f"No audio or video formats available for download for song {self.id} ({self.title})"
            )
            self.gettingPlaybackReady = False
            return
        ...
        datastore.write_file(
            key=self.id + "_downloadMeta",
            value=json.dumps(using),
            ext="json",
            byte=False,
        )
    ...
    def get_best_playback_MRL(self) -> str | None:
        ...
        if self.downloadState == DownloadState.DOWNLOADED:
            ...
            if result := cacheManager.getdataStore("song_datastore").getFilePath(
                self.id
            ):
                return result
            else:
                self.logger.error(
                    f"File for song {self.id} not found in datastore, returning empty None."
                )
                return None
        else:
            if not self.playbackInfo:
            ...
            if not self.playbackInfo.get("audio", None):
                self.logger.error(
                    f"No audio playback info found for song {self.id}, returning None."
                )
